...
func workerProbePath(path string, reportID byte) (int, bool, bool, int, error) {
	...
	allowWrites := false
	if deviceInfo != nil {
		allowWrites = canWriteToDevice(deviceInfo)
	}
	...
	// --------------------------------------------------
	if isD2W(path) {
		if lvl, chg, ok := probeD2W(d, allowWrites); ok {
...
func probeD2W(dev *hid.Device, allowWrites bool) (int, bool, bool) {
	if logger != nil {
	...
	if allowWrites {
		if data, err := requestBatteryReportWithCommand(dev); err == nil {
			if level, charging, valid := parseModelD2WirelessBatteryWorker(data); valid {
				if logger != nil {
					logger.Printf("[WORKER] probeD2W: Success via command fallback: level=%d charging=%v", level, charging)
				}
				return level, charging, true
			}
		} else if logger != nil {
			logger.Printf("[WORKER] probeD2W: Command fallback failed: %v", err)
		}
	} else if logger != nil {
		logger.Printf("[WORKER] probeD2W: Command fallback blocked (writes disabled)")
	}
